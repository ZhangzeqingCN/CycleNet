...
class ReferenceSeries:
    def __init__(self, train_data, data_len, rs_type="sin", Q=10, resolution=10, alpha=0.7, pr=False):
        self.L, self.C = train_data.shape
        ...
        self.frequency, self.fre_magnitude = self._fft(train_data)
    ...
    def pbp_extractor(self):
        ...
        assert len(
            pbp) > 0, "primary base-pattern not found, please decrease the value of alpha to relax the conditions and continue extracting"
    ...
    def base_pattern_extractor(self, manual_add=[]):
        ...
        # self.base_patterns = [(24, 1), (168, 1), (24, 2), (24, 3), (24, 4), (168, 2)]
    ...
    def reference_series_generator(self):
        assert self.rs_type in ["sin", "swatooth", "reactangle",
                                "pulse"], "rs_type must be one of sin, swatooth, reactangle, or pulse"

        if self.rs_type == "sin":
            rs = [[sin(t * 2 * pi * bp[1] / bp[0]) for t in range(self.data_len)] for bp in self.base_patterns]
        elif self.rs_type == "swatooth":
            rs = [[t * bp[1] % bp[0] for t in range(self.data_len)] for bp in self.base_patterns]
        elif self.rs_type == "reactangle":
            rs = [[2 * t * bp[1] // bp[0] % 2 for t in range(self.data_len)] for bp in self.base_patterns]
        elif self.rs_type == "pulse":
            rs = [[1 if t * bp[1] % bp[0] == 0 else 0 for t in range(self.data_len)] for bp in self.base_patterns]

        return np.array(rs, dtype=float).T
