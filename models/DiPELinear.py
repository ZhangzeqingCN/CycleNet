...
class FFTExpandBigConv1d(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.zeros((1, self.num_experts, 1, self.freq_len), dtype=torch.cfloat)
        )
...
class StaticTimeWeight(nn.Module):
    ...
    def __init__(self, input_len, num_experts):
        ...
        self.input_len = input_len
        self.num_experts = num_experts
        self.weight = nn.Parameter(torch.ones(1, self.num_experts, 1, self.input_len))
    ...
    def forward(self, x, rank_experts=None, windowing=False):
        ...
        if windowing:
            window = torch.hamming_window(
                self.input_len, dtype=x.dtype, device=x.device
            )
            x = x * window
        x = torch.fft.rfft(x)
        x = x * weight
...
class Model(nn.Module):
    ...
    def __init__(self, args: Args):
        ...
        self.individual_f = False
        self.individual_t = False
        self.individual_c = False
        self.num_experts = 1

        self.use_revin = args.use_revin
        self.use_time_w = True
        self.use_freq_w = True
        self.loss_alpha = 0.0
        self.t_loss = "mse"

        self.example_input_array = torch.Tensor(32, self.input_len, self.num_features)

        if self.num_experts > 1:
            self.route = nn.Parameter(torch.randn(1, self.num_experts, self.num_features, 1))
        ...
        self.static_route = torch.eye(self.num_features).unsqueeze(0).unsqueeze(-1)

        if self.use_time_w:
            if self.individual_t:
                self.time_w = StaticTimeWeight(self.input_len, self.num_features)
            else:
        ...
        if self.use_freq_w:
            if self.individual_f:
                self.freq_w = StaticFreqWeight(self.input_len, self.num_features)
            else:
        ...
        if self.individual_c:
            self.expert = FFTExpandBigConv1d(
                self.num_features, self.input_len, self.output_len
            )
        else:
            self.expert = FFTExpandBigConv1d(
                self.num_experts, self.input_len, self.output_len
            )
